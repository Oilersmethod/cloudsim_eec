...
void Scheduler::Init()
{
    ...
    SimOutput("Scheduler::Init(): Initializing Full-Power scheduler", 0);
 ...
 *
 * Implements the task allocation strategy of Full-Power algorithm:
 * 1. For GPU-capable tasks, prioritize machines with GPUs
 * 2. For all tasks, find a machine with compatible CPU and lowest utilization
 * 3. Ensure all machines stay in S0 state (fully powered on)
 * 4. Create VMs as needed to handle tasks
 *
 * @param now Current simulation time
 ...
 *
 * In the Full-Power algorithm:
 * 1. Prioritizes moving tasks to less loaded machines
 * 2. For GPU-capable tasks, tries to find a GPU machine
 * 3. Ensures all machines remain in S0 state
 *
 * @param time Current simulation time
    ...
    if (gpu_capable)
    {
        for (auto machine_id : sortedMachines)
        {
            ...
            if (machine_info.gpus > 0 && machine_info.cpu == cpu_type)
            {
                ...
                double target_util = CalculateCPUUtilization(machine_id);
                double current_util = CalculateCPUUtilization(current_machine);
